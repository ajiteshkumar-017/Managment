...
const emptyOverview: DepartmentOverview = {
  code: "",
  name: "",
  passRate: 0,
  avgCgpa: 0,
...
function DepartmentResultsPage() {
  ...
  const [overview, setOverview] = useState<DepartmentOverview>(emptyOverview);
  const [semesters, setSemesters] = useState<SemesterPerformance[]>([]);
  const [otherDepartments, setOtherDepartments] = useState<
    { department: string; passRate: number; totalExamTaken: number; isCurrent: boolean }[]
  >([]);
    ...
    async function load() {
      ...
      try {
        const res = await axios.get(
          `/api/admin/result/${encodeURIComponent(department)}`,
        );
        if (!res.data?.success) {
          toast.error(res.data?.message || "Failed to fetch department results");
          return;
        }
        ...
        setOverview({
          code: ov?.code || department,
          name: ov?.name || department,
          passRate: ov?.passRate ?? 0,
          avgCgpa: ov?.avgCgpa ?? 0,
          totalPassed: ov?.totalPassed ?? 0,
          totalFailed: ov?.totalFailed ?? 0,
          students: ov?.students ?? 0,
          weakSemesters: ov?.weakSemesters ?? 0,
          hod: ov?.hod || "—",
        });
        setSemesters(res.data.data?.semesters || []);
        setOtherDepartments(res.data.data?.otherDepartments || []);
      } catch (err: any) {
        if (!cancelled) {
          toast.error(
            err?.response?.data?.message || "Failed to fetch department results",
          );
          setOverview({ ...emptyOverview, code: department, name: department });
          setSemesters([]);
          setOtherDepartments([]);
        }
      } finally {
    ...
    <div className="min-h-screen w-full max-w-[100vw] overflow-x-hidden bg-linear-to-br from-slate-50 via-white to-slate-50 flex flex-col lg:flex-row">
      ...
      <div className="flex-1 min-w-0 w-full">
        <div className="overflow-hidden bg-white p-5 text-slate-900 shadow-sm sm:p-6 md:p-7 lg:p-6">
          <div className="flex flex-col gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-start sm:justify-between sm:pb-8">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-slate-600">
                ...
                {overview.hod && overview.hod !== "—" ? ` · HOD: ${overview.hod}` : ""}
              </p>
            </div>
            <button
              type="button"
          ...
          {loading ? (
            <p className="mt-8 text-sm text-slate-500">Loading department results…</p>
          ) : (
            <>
              ...
              {otherDepartments.length > 0 && (
                <div className="mt-10">
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Switch department to compare semester performance
                  </p>
                  <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5">
                    {otherDepartments.map((dept) => {
                      const isCurrent = dept.isCurrent;
                      const card = (
                        <div
                        ...
                        >
                          <p className="text-xs font-medium uppercase tracking-wide text-slate-500">
                            {isCurrent ? "Current" : "Department"}
                          </p>
                          <p className="mt-2 text-xl font-bold text-slate-900">{dept.department}</p>
                          <p className="mt-1 text-sm text-slate-600">{dept.passRate}% pass</p>
                        </div>
                      );

                      if (isCurrent) {
                        return <div key={dept.department}>{card}</div>;
                      }
              ...
              <div className="mt-10">
                ...
                {semesters.length === 0 ? (
                  <p className="mt-6 text-sm text-slate-500">No published semester results for this department.</p>
                ) : (
                  <>
