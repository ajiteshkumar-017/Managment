...
function SemesterSubjectsPage() {
    ...
    async function load() {
      ...
      try {
        const res = await axios.get(
          `/api/admin/result/${encodeURIComponent(department)}/semester/${encodeURIComponent(semester)}`,
        );
        if (!res.data?.success) {
          toast.error(res.data?.message || "Failed to fetch semester results");
          return;
        }
        ...
        if (!cancelled) {
          toast.error(
            err?.response?.data?.message || "Failed to fetch semester results",
          );
          setSubjects([]);
        }
      } finally {
  ...
  const poorCount = subjects.filter((s) =>
    getPerformanceLevel(s.passRate, Boolean(s.hasResults)) === "poor",
  ).length;
  const warningCount = subjects.filter((s) =>
    getPerformanceLevel(s.passRate, Boolean(s.hasResults)) === "warning",
  ).length;
  const avgPass = useMemo(
    () => {
      const withResults = subjects.filter((s) => s.hasResults);
  ...
  const handleExport = () => {
    setExporting(true);
    try {
      exportSemesterReport({ department, semester, subjects });
    ...
    <div className="min-h-screen w-full max-w-[100vw] overflow-x-hidden bg-linear-to-br from-slate-50 via-white to-slate-50 flex flex-col lg:flex-row">
      ...
      <div className="flex-1 min-w-0 w-full">
        <div className="overflow-hidden bg-white p-5 text-slate-900 shadow-sm sm:p-6 md:p-7 lg:p-6">
          ...
          {loading ? (
            <p className="mt-8 text-sm text-slate-500">Loading semester results…</p>
          ) : (
            <>
              ...
              <div className="mt-10">
                ...
                {subjects.length === 0 ? (
                  <p className="mt-6 text-sm text-slate-500">No published subject results for this semester.</p>
                ) : (
                  <>
